(() => {
    ...
    function nvl(num, numIfNo) {
        return !num && num !== 0 ? numIfNo : num;
    }
    function nvlArr(arr, arrIfNo) {
        return !arr || !Array.isArray(arr) || arr.length < 1 || arr.length > 3
            ? arrIfNo
            : arr;
    }
    ...
    function createCardsGrid() {
        ...
        for (let i = 0; i < game.cards.length; i++) {
            ...
            gameCardFront.classList.add(
                "game__card--front",
                "bg-" + game.cards[i].bgNum
            );

            gameCardBack = document.createElement("div");
            gameCardBack.classList.add(
                "d-flex",
    ...
    function updatePlayersBlock() {
        ...
        if (!game.started) {
            ...
            // ? процесс игры
            content = `Ход игрока: <b>${game.activePlayer()}</b> (счет - ${
            ...
            game.players.forEach((player) => {
                ...
                accuracy +=
                    player.name +
                    " - " +
                    Number((player.success * 100) / player.attempts).toFixed(
                        2
                    ) +
                    "%"; //! надо убрать NaN!
            });
        ...
        playerElement && (playerElement.innerHTML = content);
    }
    ...
    function updateGameStatusBlock() {
        ...
        let content;
        const gameLengthContent =
            "<br>Длительность: " + getTimeStr(game.getGameTime());
        ...
        if (!game.started) {
            ...
            if (!game.isTimerOut()) {
                content =
                    "Осталось времени: " + getTimeStr(game.gameLeftSeconds);
            }
        } else {
            // ? конец игры
            if (!game.isTimerSet()) {
    ...
    const game = {
        ...
        pairCounter: {
            // механизм расчета хода (из 2х открывваемых карточек)
        ...
        tickSecond() {
            this.gameLeftSeconds = Math.max(0, this.gameLeftSeconds - 1);
        }, // вычесть одну секунду (без снижения ниже 0)
        timer: null, // таймер
        startTimer(handler) {
        ...
        isTimerSet() {
            return this.gameLengthSeconds > 0;
        }, // игра на время
        isTimerOut() {
            return this.isTimerSet() && this.gameLeftSeconds === 0;
        }, // время вышло!
        // ----------------------------------------
        ...
        nextFirstPlayer() {
            // выбрать следующего игрока из списка для передачи права первого хода
        ...
        activePlayer() {
            return this.players[this.activePlayerIndex].name;
        }, // имя игрока с правом текущего хода
        nextPlayer() {
            // переход хода следующему игроку (по умолчанию к первому)
        ...
        // ----------------------------------------
        reset() {
            // сохранение основных параметров текущей игры + сброс
            ...
            (this.cards = []), this.pairCounter.reset();
            this.attempts = 0;
            this.cardsOpened = 0;
            this.started = false;
            this.stopped = false;
    ...
    const interface = {
        ...
        isCardOpened(cardElement) {
            return !cardElement.classList.contains("is-flipped");
        },
        //
        lockCard(card) {
            this.getElement(card).classList.add("is-blocked");
        },
        lockCards() {
            game.cards.forEach((card) => this.lockCard(card));
        },
        isCardBlocked(cardElement) {
            return cardElement.classList.contains("is-blocked");
        },
        //
        getCard(cardElement) {
            return game.cards.find(
                (item) => item.num === parseInt(cardElement.id)
            );
        },
        getElement(card) {
            return document.getElementById(card.num + "-card");
        },
        //
        handleRestartButton(handler) {
            document
                .getElementById("restart")
                .addEventListener("click", handler);
        },
        hideRestartButton() {
            document.getElementById("restart").classList.add("display-hidden");
        },
        showRestartButton() {
            document
                .getElementById("restart")
    ...
    function takeCard(cardElement) {
        ...
        function compare() {
            ...
            if (card.pairNum === game.pairCounter.card.num) {
                ...
                interface.closeCard(
                    interface.getElement(game.pairCounter.card)
                );
                game.nextPlayer();
            }
    ...
    function restartGame() {
        ...
        startGame(
            game.lastCardsCount,
            game.lastPlayersNames,
            game.lastGameLengthSeconds
        );
    }
